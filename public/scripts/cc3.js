  ...
  $.ajax({
    ...
    success: function(response){
      ...
      for (var i = 0; i < response.length; i++) {
        $('#outputDiv').append('<strong>' + response[i].whoseJoke + ':' + '</strong>' + '<p>' + response[i].jokeQuestion + '</p>' + '<p>' + response[i].punchLine + '</p>');
      }//end for loop
      },
      error: function (){
...
function displayNewJoke (array){
    $('#outputDiv').append('<strong>' + array[array.length - 1].whoseJoke + ':' + '</strong>' + '<p>' + array[array.length - 1].jokeQuestion + '</p>' + '<p>' + array[array.length - 1].punchLine + '</p>');
}; //end display joke
